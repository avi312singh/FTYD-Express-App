  ...
  if (req.query.name) {
    pool.getConnection((err, connection) => {
      try {
        ...
        connection.query(
          `INSERT INTO playerInfo (playerName, online) VALUES (?, 1) ON DUPLICATE KEY UPDATE totalTime = totalTime + .25, totalTimeDaily = totalTimeDaily + .25, totalTimeWeekly = totalTimeWeekly + .25, totalTimeMonthly = totalTimeMonthly + .25`,
          [name],
          (err, result, fields) => {
            if (err) console.log(err);
            if (result) {
