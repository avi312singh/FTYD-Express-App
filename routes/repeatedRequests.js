    ...
    const secondJob = schedule.scheduleJob(
      ...
      async (fireDate) => {
          ...
          while (z--) {
            ...
            if (playerHasLeft) {
              ...
              const endpointRequest = axios.post(
                `${endpoint}serverstats/lastLogin?name=${encodeURIComponent(oldPlayers[z].name)}`,
                {},
                axiosBasicAuthConfig,
              );
        ...
        if (
          ...
          while (y--) {
            ...
            if (playerHasJoined) {
              ...
              const endpointRequest = axios.post(
                `${endpoint}serverstats/?name=${encodeURIComponent(newPlayers[y].name)}`,
                {},
                axiosBasicAuthConfig,
              );
              postRequests.push(endpointRequest);
        ...
        for (i = 0; i < newPlayers.length; i++) {
          ...
          if (
            ...
            if (
              ...
              const endpointRequest = axios.post(
                `${endpoint}serverstats/pointsSpent?name=${encodeURIComponent(newPlayers[newPlayerIndex].name)}&pointsSpent=${scoreDifference >= 90 ? 0 : scoreDifference}`,
                {},
                axiosBasicAuthConfig,
              );
              postRequests.push(endpointRequest);
              ...
              const endpointRequest = axios.post(
                `${endpoint}serverstats/kills?name=${encodeURIComponent(newPlayers[newPlayerIndex].name)}&kills=${scoreDifference % 2 == 0 ? scoreDifference / 2 : scoreDifference}`,
                {},
                axiosBasicAuthConfig,
              );
              postRequests.push(endpointRequest);
            ...
            const endpointRequest = axios.post(
              `${endpoint}serverstats/?name=${encodeURIComponent(newPlayers[newPlayerIndex].name)}`,
              {},
              axiosBasicAuthConfig,
            );
            postRequests.push(endpointRequest);
        ...
        Promise.all(postRequests)
          .then(
            console.log(
              chalk.whiteBright("Sent remaining players to database"),
            ),
          )
          .catch(console.error);
